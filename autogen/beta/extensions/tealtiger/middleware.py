...
# AG2 Beta imports (always available â€” this Extension lives in the AG2 repo)
...
class TealTigerMiddleware(BaseMiddleware):
    ...
    Args:
        ...
        mode: Governance mode â€” observe, monitor, or enforce.
    ...
    async def on_turn(
        ...
        if self.agent_id in self._frozen_agents:
            ...
            if self.mode == GovernanceMode.ENFORCE:
                ...
                raise GovernanceDenyError(f"Agent '{self.agent_id}' is frozen. Decision: {decision.decision_id}")
    ...
    async def on_tool_execution(
        ...
        if action == DecisionAction.ALLOW:
            for policy in self.policies:
                if policy.type == "secret_detection" and self._detect_secrets(tool_args):
                    action = DecisionAction.DENY
        ...
        if self.mode == GovernanceMode.OBSERVE:
            ...
            if action == DecisionAction.ALLOW:
                ...
                return f"[GOVERNANCE PENDING] Tool '{tool_name}' requires approval. Decision ID: {decision.decision_id}"
    ...
    def freeze(self, agent_id: str | None = None) -> None:
        """Freeze an agent â€” blocks ALL actions regardless of mode."""
        target = agent_id or self.agent_id
        self._frozen_agents.add(target)
        logger.info("[GOVERNANCE] Frozen agent: %s", target)

    def unfreeze(self, agent_id: str | None = None) -> None:
        """Unfreeze an agent â€” restores normal governance evaluation."""
        target = agent_id or self.agent_id
        self._frozen_agents.discard(target)
        logger.info("[GOVERNANCE] Unfrozen agent: %s", target)

    def is_frozen(self, agent_id: str | None = None) -> bool:
